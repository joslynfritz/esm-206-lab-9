...
# When looking at data what questions come up as you check it out? 
...
# Every step of the way look at the data and see what stands out. Here Santa Maria and SLO are the most dense. 
```
...
# Allison says that these generally look linear, so we're okay to use the linear regression. 
...
```{r saturated}
# NOTE: it makes no sense to add price per sqft because it will clearly show a linear relationsip between because price is embedded in price per sqft. this can lead to biased overfitting of model results. 
# bedroom, bathroom, and sqft all dsecribe size, lets just use sqft 

# a. Saturated model: 
homes_lm1 <- lm(Price ~ City + SqFt + Bedrooms + Bathrooms + PricePerSqFt + Status, data = homes_sub)
...
# This represents, that if all other variables remain the same, a house in SLO will be $34,000 more than in Arroyo Grande (since AG is the baseline)
# it describes that for every 1 sqft increase the price should increase by $245.9
...
lm function says if you feed it qualitative variable it'll make it numeric and easier to deal with. 
...
# Look at Residuals vs Fitted graph, QQplot looks VERY normally distributed

# Overall heterstadsticity os Okay, residuals normality definitely looks good, and conceptually and mathematically my model is making sense! 
```
...
# Lower value indicates a better model, and rn lm1 is lower than lm3 BUT that doesn't mean it's the better dataset. 

# Answer: which would you pick? 
# I would still pick homes_lm3 because of all the above reasons. "Overall heterstadsticity os Okay, residuals normality definitely looks good, and conceptually and mathematically my model is making sense! "
